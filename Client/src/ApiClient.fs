...
let private parseLinkObj (o: obj) : Link =
  ...
  let sortOrder =
    getFirst<int> o ["sortOrder"; "SortOrder"]
...
let private parseUserProfileObj (o: obj) : UserProfile =
  let firstName =
    getFirst<string> o ["firstName"; "FirstName"]
    |> Option.defaultValue ""
  let lastName =
    getFirst<string> o ["lastName"; "LastName"]
    |> Option.defaultValue ""
  let displayEmail = getFirst<string> o ["displayEmail"; "DisplayEmail"]
  let profileSlug =
    getFirst<string> o ["profileSlug"; "ProfileSlug"]
    |> Option.defaultValue ""
...
let private realProfileApi: ProfileApi = {
  GetProfile =
    fun () ->
      async {
        let! res = Http.request "/api/profile" |> Http.method GET |> Http.send
  ...
  SaveProfile =
    fun (profile: UserProfile) ->
      async {
        ...
        if
          res.statusCode = 204
          || (is2xx res.statusCode && bodyOrEmpty res.responseText = "")
        then
          return Ok ()
        else
...
let private realLinkApi: LinkApi = {
  GetLinks =
    fun () ->
      async {
        let! res = Http.request "/api/links" |> Http.method GET |> Http.send
        let status = res.statusCode
        let body = bodyOrEmpty res.responseText
        if not (is2xx status) then
          return Error (sprintf "HTTP %d: %s" status body)
